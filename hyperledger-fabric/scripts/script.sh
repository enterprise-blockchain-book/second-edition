...
setGlobals () {
	...
	if [ $1 -eq 0 -o $1 -eq 1 ] ; then
		...
		if [ $1 -eq 0 ]; then
			...
			CORE_PEER_CHAINCODELISTENADDRESS=peer0.app.fte.com:7052
		else
...
installChaincode () {
	PEER=$1
	setGlobals $PEER
	peer chaincode install -n tradefinancecc -v 1.0 -l $LANGUAGE -p $CC_PATH >&log.txt
	res=$?
	cat log.txt
...
instantiateChaincode () {
	...
	setGlobals $PEER
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer.fte.com:7050 -C $CHANNEL_NAME -n tradefinancecc -l $LANGUAGE -v 1.0 -c '{"Args":["init","FTE_2","FTE_B_1","FTE_S_1","SKU001","10000","1000"]}' -P "OR	('Org1FTE.member','Org2BNK.member','Org3SHP.member')" >&log.txt
	else
		peer chaincode instantiate -o orderer.fte.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n tradefinancecc -l $LANGUAGE -v 1.0 -c '{"Args":["init","FTE_2","FTE_B_1","FTE_S_1","SKU001","10000","1000"]}' -P "OR ('Org1FTE.member','Org2BNK.member','Org3SHP.member')" >&log.txt
	fi
	res=$?
	cat log.txt
